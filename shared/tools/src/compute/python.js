...
OpenMeasures pipeline pattern: large JSON from an API call → write to /workspace/data.json → run_python reads it locally → returns analysis. Avoids context window bloat.`,
    ...
    handler: async (input, context) => {
        ...
        const workspaceRoot = (0, workspace_helper_1.getWorkspaceRoot)();
        ...
        if (!input.background) {
            ...
            try {
                const stdout = (0, child_process_1.execSync)(`docker ${dockerArgs.join(' ')}`, {
                    timeout: timeoutMs,
        ...
        // --- BACKGROUND (long job) ---
        const jobId = (0, crypto_1.randomUUID)().slice(0, 8);
        // Start detached container
        let containerId;
        try {
            containerId = (0, child_process_1.execSync)(`docker ${dockerArgs.join(' ')}`, { encoding: 'utf-8' }).trim();
        }
        catch (err) {
            cleanTempFiles(workspaceRoot);
...
function cleanTempFiles(workspaceRoot) {
    try {
...
`.trim();
    const watcher = (0, child_process_1.spawn)(process.execPath, ['-e', watcherCode], {
        detached: true,
        stdio: 'ignore',
    });
    watcher.unref();
}
async function postToMattermost(context, message) {
    if (!context.room_id || !context.credentials.MATTERMOST_URL || !context.credentials.MATTERMOST_BOT_TOKEN)
        return;
    try {
